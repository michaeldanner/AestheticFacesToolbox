...
if __name__ == "__main__":
    ...
    # path = 'example/'
    load = config.file['images']
    image = config.file['image_folder']
    print(image)
    f = config.file['annotations']
    out = config.file['output']
    age = config.file['ages']
    dm = DataMatrix(load, f, image, out, age, 9)

    print("\nLoading image dataset")
    line_count, data = dm.load_dataset()
    print("{} images found".format(line_count))
    print(data)

    print("\nGenerating the datamatrix")
    dm.generate_datamatrix()

    dm.estimate_images()
    dm.normal_rank_graph()
    dm.get_age_data()
